...
interface CommunicationModuleProps {
  ...
  onDeleteAnnouncementDB?: (id: string) => Promise<void>;
  userRole?: UserRole;
  currentUserId?: string;
}

const CommunicationModule: React.FC<CommunicationModuleProps> = ({
  announcements, setAnnouncements, classes, editMode, onWatchModeAttempt, onSuccess, onDeleteAnnouncementDB, userRole, currentUserId
}) => {
  ...
  const deleteAnnouncement = async (id: string) => {
    ...
    // Supabase'den de sil
    if (onDeleteAnnouncementDB) {
      try {
        await onDeleteAnnouncementDB(id);
      } catch (err) {
        console.error('Duyuru Supabase silme hatası:', err);
      }
    }
