...
import { getBranchColor, standardizeBranchCode } from '../utils';
...
const SchedulingModule: React.FC<SchedulingModuleProps> = ({ teachers, classes, lessons, onApprove, setActiveModule, schoolConfig }) => {
      ...
      if (loading || saving) {
         ...
         timerRef.current = window.setInterval(() => {
            ...
            if (saving) {
               ...
               if (diff < 15000) setStatusMessage('HASSAS DNA ANALİZİ VE DAĞITIM...');
               else if (diff < 45000) setStatusMessage('GÜNLÜK DERS DENGESİ HESAPLANIYOR...');
               else if (diff < 90000) setStatusMessage('MATEMATİKSEL KONTROLLER YAPILIYOR...');
               else setStatusMessage('ZORLU KOMBİNASYONLAR ÇÖZÜMLENİYOR...');
            }
         }, 200);
      } else {
   ...
   const handleRunAI = async () => {
      ...
      try {
         ...
         const msg = error?.message || error?.toString() || 'Bilinmeyen hata';
         setFailureReason(`MOTOR_HATASI: ${msg}`);
         setLoading(false);
      }
   };
   ...
   const handleApply = (jsonText: string) => {
      try {
         ...
         // POST-PROCESSING: Sınıf bazlı fazla slot temizliği
         const validatedArray: ScheduleEntry[] = [];
         const uniqueClasses = Array.from(new Set(normalizedArray.map(s => s.sinif))).filter(c => c);

         for (const className of uniqueClasses) {
            const classData = classes.find(c => c.name === className);
            const maxHours = classData?.assignments?.reduce((s: number, a: any) => s + (a.hours || 0), 0) || 0;
            const classEntries = normalizedArray.filter(s => s.sinif === className);

            if (maxHours > 0 && classEntries.length > maxHours) {
               // Fazla slot var! Sadece atanmış derslere ait olanları koru
               const assignedLessons = new Set(
                  (classData?.assignments || []).map((a: any) => {
                     const lesson = lessons.find(l => l.id === a.lessonId);
                     return lesson?.name?.toUpperCase();
                  }).filter(Boolean)
               );

               // Önce atanmış derslere ait olanlar, sonra kalanlar
               const matched = classEntries.filter(e => assignedLessons.has(e.ders?.toUpperCase()));
               const unmatched = classEntries.filter(e => !assignedLessons.has(e.ders?.toUpperCase()));

               // Atanmış derslerden maxHours kadar al, gerisi kesilir
               validatedArray.push(...matched.slice(0, maxHours));
               // Eğer hâlâ eksik varsa unmatched'den tamamla
               if (matched.length < maxHours) {
                  validatedArray.push(...unmatched.slice(0, maxHours - matched.length));
               }

               console.log(`[SENKRON] ${className}: ${classEntries.length} → ${Math.min(classEntries.length, maxHours)} slot (max: ${maxHours})`);
            } else {
               validatedArray.push(...classEntries);
            }
         }

         setAppliedSchedule(validatedArray);
         setLoading(false);
                                 ...
                                 if (entry) {
                                    ...
                                    if (teacherObj) {
                                       ...
                                       if (parts.length >= 2) {
                                          ...
                                          displayTeacher = `${initial}.${surname}`;
                                       } else {
